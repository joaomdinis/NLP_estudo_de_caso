...
class ClearWords():
    ...
    def removeSlang(self, label):
        with open(r'termos.json', encoding='utf-8') as json_file:
    ...
    def removerAcentos(self, label):
        self.df[label] = self.df[label].apply(lambda x: ' '.join([normalize('NFKD', w).encode('ASCII', 'ignore').decode('ASCII') for w in x.split()]).strip())
        return self.df[label]
